...
- **임베딩 교체형** — 기본 `bge-m3`는 다국어(100+ 언어, 영어·한국어 모두 강함)다. 설정
  한 줄로 7종(Ollama·LM Studio·llama.cpp·TEI·OpenAI·Voyage·Gemini) 중 무엇으로든 전환,
  모델별 인덱스 자동 재빌드.
- **증분 인덱싱** — 변경된 노트만 다시 임베딩, 클라이언트는 파일만 쓰면 된다. 노트를 고치면
  그 노트가 낀 **관계 엣지도 자동 재평가**돼 낡은 판정이 남지 않는다.
...
- **엣지**: 초록 = `[[위키링크]]`, 회색 = 의미 유사, 빨강 = 정리 후보(중복), 그리고
  **라벨 관계 엣지** — 청록 *지지*, 주황 *반박*, 보라 *확장*(`POST /classify-relations`로 채움 —
  노트를 고치면 그 노트가 낀 관계는 떨어져, 낡은 판정을 유지하는 대신 다음 실행에서 다시 평가된다)
- **노드 클릭** → 상세 패널(제목·유형·태그·본문·연결 노트). 연결 칩이나 `/ask` 근거를
  클릭하면 그 노드로 점프
- **질문 박스**(좌하단)가 `/ask`를 내 뇌에 던진다
- **언어 토글**(우상단) — UI를 한국어/영어로 전환(브라우저 언어 자동 감지, 선택 기억).
  유형·관계 라벨은 매핑되지만(예: `의미 → Concept`), 노드 제목·본문은 작성한 언어 그대로
  둔다(내 콘텐츠는 자동 번역하지 않는다)
